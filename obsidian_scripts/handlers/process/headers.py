...
def add_metadata_to_yaml(filepath, tags, summary, category, subcategory, status):
    ...
    """
    try:
        logging.debug("[DEBUG] add_yaml : démarrage fonction : %s %s",filepath, status)
        logging.debug("[DEBUG] add_yaml : démarrage fonction : %s / %s", category, subcategory)
        ...
        try:
            logging.debug("[DEBUG] add_yaml : nouvelle entête : %s / %s",category, subcategory)
            logging.debug("Add_Metadata_to_yaml : Préparation à écrire : %s",yaml_block)
            # Sauvegarde dans le fichier
            with open(filepath, "w", encoding="utf-8") as file:
                file.writelines(lines)
            logging.info("[INFO] Génération de l'entête terminée")
            logging.debug("Add_Metadata_to_yaml Écriture réussie dans le fichier")
        except FileNotFoundError as e:
            logging.error("Erreur lors de l'écriture dans le fichier : %s",e, exc_info=True)
    except Exception as e:  # nosec: catch-all
        logging.error("[ERROR]Fichier non trouvé : %s", e)

def make_properties(content, filepath, category, subcategory, status):
    """
    Génère les entêtes et met à jour les métadonnées.
    """
    logging.debug("[DEBUG] make_pro : Entrée de la fonction")
    ...
    logging.debug("[DEBUG] make_pro : Récupération des tags et résumé")
    tags = get_tags_from_ollama(content)
    summary = get_summary_from_ollama(content)

    # Mise à jour des métadonnées YAML
    logging.debug("[DEBUG] make_pro : Mise à jour du YAML")
    ...
    with open(filepath, "r+", encoding="utf-8") as file:
        ...
        logging.debug("[DEBUG] make_pro : Recalcul du nombre de mots")
        ...
        for i, line in enumerate(lines):
            if line.startswith("word_count:"):
                ...
                logging.debug("[DEBUG] make_pro : Mise à jour de word_count existant")
                break

        if not word_count_updated:
            # Ajout du champ `word_count` s'il n'existe pas
            logging.debug("[DEBUG] make_pro : Ajout du champ word_count pour")
    ...
    logging.debug("[DEBUG] make_pro : Écriture réussie et fichier mis à jour")
...
def check_type_header(filepath):
    ...
    """
    try:
        logging.debug("[DEBUG] check_type démarrage fonction")
        with open(filepath, "r", encoding="utf-8") as file:
            lines = file.readlines()
        # Vérification de l'entête YAML
        yaml_start, yaml_end = -1, -1
        if lines[0].strip() == "---":
            ...
            yaml_end = next((i for i, line in enumerate(lines[1:], start=1)
                             if line.strip() == "---"), -1)
            if yaml_end != -1:
                logging.debug("[DEBUG] add_yaml : entête détectée %s à %s", yaml_start, yaml_end)
        ...
        logging.error("Erreur lors du traitement de l'entête YAML pour %s : %s",filepath, e)
    return None
