    ...
    if len(backup_files) > MAX_BACKUPS:
        ...
        for i in range(num_to_delete):
            try:
                ...
                logging.info(f"[INFO] Suppression de l'ancienne sauvegarde : {backup_files[i]}")
            except Exception as e:
                logging.error(f"[ERREUR] Impossible de supprimer {backup_files[i]} : {e}")
...
def backup_note_paths():
    ...
    while True:
        ...
        try:
            ...
            logging.info(f"[INFO] Sauvegarde automatique créée : {backup_file}")
            rotate_backups()

        except Exception as e:
            logging.error(f"[ERREUR] Impossible de créer la sauvegarde : {e}")
